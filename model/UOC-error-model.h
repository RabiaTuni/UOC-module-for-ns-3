#ifndef UOC_ERROR_MODEL_H
#define UOC_ERROR_MODEL_H
...
class UOC_ErrorModel : public RateErrorModel	// This class is a subclass of RateErrorModel class
{
	public:
		// Public methods
		static TypeId GetTypeId (void); // returns meta-information about UOC_ErrorModel class 

		UOC_ErrorModel ();	// constructor
		virtual ~UOC_ErrorModel ();	//destructor
		...
		UOC_ErrorModel::ModScheme GetScheme (void) const;	// returns the modulation scheme used
		...
		double CalculatePacketErrorRate ();  //for calculating the PER
	...
	private:
		...
		double Q;		// Bit-error-rate

		Ptr<RandomVariableStream> m_ranvar;		// random variable stream
		UOC_ErrorModel::ModScheme mod_scheme;	// the modulation scheme used
};
